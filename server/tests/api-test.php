...
class ApiTest extends TestCase
{
	...
	public function testGenerateNonce()
	{
		...
		$nonceHex = $this->api->generateNonce();

		// Check it is the correct length (512 / 4 = 128)
		$this->assertEquals(128, strlen($nonceHex), 'Failed length test: ' . $nonceHex);
	}
	...
	public function testValidateDataNonce()
	{
		...
		$nonceHex = $this->api->generateNonce();
		$timestamp = time();
		$result = $this->api->addSentNonceToDatabase($nonceHex, $timestamp);
		$this->assertTrue($result);
	}
	...
	public function testCleanupOldNonces()
	{
		...
		$this->db->preparedUpdate('DELETE FROM nonces');
	...
	public function testSaveMessageToDatabase()
	{
		...
		$result = $this->db->preparedUpdate('DELETE FROM messages');
