...
class Api
{
	...
	public function performCleanup()
	{
		...
		$result = $this->db->preparedSelect('SELECT cleanup_last_run FROM settings');
	...
	public function cleanupReadMessages()
	{
		...
		for ($i = 0, $numOfUsers = count($this->users); $i < $numOfUsers; $i++)
		{
			...
			$readByParam = 'read_by_' . $this->users[$i];
	 ...
	 * @param string $from Which user the message is from e.g. alpha, bravo etc which is whitelisted
	...
	public function saveMessageToDatabase($from, $message)
	{
		...
		$query = "INSERT INTO messages (from_user, message, $readByUser) "
	 ...
	 * @param string $requestFrom Who the request is from that is asking for new messages (alpha etc from whitelist)
	 * @return array Returns an array containing keys 'success', 'statusMessage' and 'messages' if there were messages
	 */
	 ...
	 * response won't be authenticated with a valid MAC. The error message in the header is only displayed
	 * in debug mode which needs to be manually enabled in the group server configuration.
	...
	public function outputErrorResponse($headerErrorMessage)
	{
		...
		if ($this->applicationConfig['testResponseHeaders'] === true)
		{
			...
			// Otherwise in production normally just output a 200 header and stop execution
