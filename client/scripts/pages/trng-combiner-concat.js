...
var trngConcatCombinerPage = {
	...
	{
		// Init
		trngConcatCombinerPage.initResetButton();
		trngConcatCombinerPage.initBrowseFilesButton();
		trngConcatCombinerPage.initExportFileButton();
	},
	...
	initBrowseFilesButton: function()
	{
		...
		query.getCached('.jsFileLoader').on('change', function()
		{
			...
			// Set for use later
			trngConcatCombinerPage.totalNumFiles = totalNumFiles;

			// For every file
			for (var i = 0; i < files.length; i++)
			{
				// Load the file information
				trngConcatCombinerPage.loadFileInformation(files[i]);
			}
		});
	},
	...
	loadFileInformation: function(file)
	{
		...
		reader.onload = function(event)
		{
			...
			trngConcatCombinerPage.allDataHex += loadedHexSymbols;
			trngConcatCombinerPage.numFilesLoaded += 1;
			trngConcatCombinerPage.rowHtml += $row.prop('outerHTML');

			// If all the files are loaded
			if (trngConcatCombinerPage.numFilesLoaded === trngConcatCombinerPage.totalNumFiles)
			{
				...
				query.getCached('.jsFileInfoTable').append(trngConcatCombinerPage.rowHtml).show();

				// Find the number of hexadecimal symbols and bytes loaded
				var totalHexSymbolsLoaded = trngConcatCombinerPage.allDataHex.length;
				...
				if ((totalBytes % 2) !== 0)
				{
					...
					trngConcatCombinerPage.allDataHex = trngConcatCombinerPage.allDataHex.slice(0, -2);
				...
				app.showStatus('success', (
						'All data loaded. ' +
						'Total files: ' + trngConcatCombinerPage.numFilesLoaded + '. ' +
	...
	initExportFileButton: function()
	{
		...
		{
			// Convert to Base64
			var outputBase64 = common.convertHexadecimalToBase64(trngConcatCombinerPage.allDataHex);
	...
	cleanup: function()
	{
		...
		trngConcatCombinerPage.numFilesLoaded = 0;
		trngConcatCombinerPage.totalNumFiles = 0;
		trngConcatCombinerPage.allDataHex = '';
		trngConcatCombinerPage.rowHtml = '';
	}
};
