...
var decoy = {
	...
	sendDecoyMessageToServer: function(randomPadIdentifier, decoyMessage, triggerTimeMilliseconds)
	{
		...
		common.sendRequestToServer(data, serverAddressAndPort, serverKey, function(validResponse, responseData)
		{
			...
			if (validResponse)
			{
				// If it saved to the server database
				if (responseData.success)
				{
					// Log useful information to console
					console.info('Decoy message ' + randomPadIdentifier + ' successfully sent to server at ' + common.getCurrentLocalTime() + ' after ' + (triggerTimeMilliseconds / 1000) + 's random delay.');
				}
				else {
			...
			else if (validResponse === false)
			{
				...
				app.showStatus('error', 'Error sending decoy message to server. Check: 1) you are connected to the network, 2) the client/server configurations are correct, and 3) client/server system clocks are up to date. If everything is correct, the data may have been tampered with by an attacker.');
			}
		});
	},
